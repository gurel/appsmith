...
public class GrpcEndpoint {
    ...
    public GrpcEndpoint(Descriptors.MethodDescriptor method) {
        ...
        this.gMethodDescriptor = io.grpc.MethodDescriptor.<DynamicMessage, DynamicMessage>newBuilder()
                .setType(
                        clientStreaming
                                ? (serverStreaming ? BIDI_STREAMING : CLIENT_STREAMING)
    ...
    public Mono<ActionExecutionResult> call(
            Channel channel, DynamicMessage message, CallOptions callOptions, ObjectMapper objectMapper) {
                    ...
                    try {
                        if (serverStreaming) {
                            ...
                            for (DynamicMessage m : protoTree) {
                                responses.add(JsonFormat.printer()
                                        .includingDefaultValueFields()
                                        .print(m));
                            }
                            bodyString = "[" + String.join(",", responses) + "]";
                        } else {
                            bodyString = JsonFormat.printer()
                                    .includingDefaultValueFields()
                                    .print(protoTree.get(0));
                        }
                        log.info("Gurel Response", bodyString);
    ...
    @Slf4j
    private static class ResponseStream implements StreamObserver<DynamicMessage> {
    ...
    private static class HeaderCapture extends ClientCall.Listener {
