...
public class GrpcPlugin extends BasePlugin {
    ...
    @Extension
    public static class GrpcPluginExecutor
            implements PluginExecutor<ReflectiveManagedChannel>, SmartSubstitutionInterface {
        ...
        public Mono<ActionExecutionResult> executeParameterized(
            ...
            try {
                String body = getDataValueSafelyFromFormData(
                        actionConfiguration.getFormData(), FORM_DATA_BODY, STRING_TYPE, "");
                ...
                errorResult.setStatusCode(
                        AppsmithPluginError.PLUGIN_ERROR.getAppErrorCode().toString());
                errorResult.setIsExecutionSuccess(false);
                errorResult.setErrorInfo(e);
                return Mono.just(errorResult);
            }
            String serviceName = getDataValueSafelyFromFormData(
                    actionConfiguration.getFormData(), FORM_DATA_SERVICE, STRING_TYPE, "");
            String endpointName = getDataValueSafelyFromFormData(
                    actionConfiguration.getFormData(), FORM_DATA_ENDPOINT, STRING_TYPE, "");
            String body =
                    getDataValueSafelyFromFormData(actionConfiguration.getFormData(), FORM_DATA_BODY, STRING_TYPE, "");

            if (body == null || "".equals(body)) {
                return Mono.error(
                        new AppsmithPluginException(AppsmithPluginError.PLUGIN_ERROR, "Body cannot be empty"));
            }

            Optional<GrpcEndpoint> first = connection.getGrpcServices().stream()
                    .filter((end) -> end.getMethod().getService().getFullName().equals(serviceName)
                            && end.getMethod().getName().equals(endpointName))
                    .findFirst();
            if (first.isEmpty()) {
                return Mono.error(
                        new AppsmithPluginException(AppsmithPluginError.PLUGIN_ERROR, "Cannot find endpoint"));
            }
            GrpcEndpoint grpcEndpoint = first.get();
            DynamicMessage.Builder requestBuilder = DynamicMessage.getDefaultInstance(
                            grpcEndpoint.getMethod().getInputType())
                    .newBuilderForType();
            try {
                JsonFormat.parser().merge(body, requestBuilder);
            } catch (InvalidProtocolBufferException e) {
                log.error(body);
                log.error(grpcEndpoint
                        .getMethod()
                        .getInputType()
                        .getFields()
                        .get(0)
            ...
            return Mono.fromCallable(() -> {
                        switch (request.getRequestType()) {
                            ...
                            case "ENDPOINT_SELECTOR":
                                Set<String> endpointList = connection.getEndpoints(
                                        (String) request.getParameters().get("service"));
        ...
        public Set<String> validateDatasource(DatasourceConfiguration datasourceConfiguration) {
            ...
            if (datasourceConfiguration == null
                    ...
                    || datasourceConfiguration.getUrl().equals("")) {
        ...
        public Mono<DatasourceTestResult> testDatasource(DatasourceConfiguration datasourceConfiguration) {
            return channelBuilder(datasourceConfiguration)
                    .map(chanel -> {
