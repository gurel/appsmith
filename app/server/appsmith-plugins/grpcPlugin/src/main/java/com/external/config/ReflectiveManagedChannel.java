...
public class ReflectiveManagedChannel extends ManagedChannel implements StreamObserver<ServerReflectionResponse> {
    ...
    private static final List<String> IGNORED_SERVICES =
            List.of("grpc.reflection.v1alpha.ServerReflection", "grpc.health.v1.Health");
    ...
    ManagedChannel buildChannel(DatasourceConfiguration configuration) throws IOException {
        ...
        boolean useSelfSigned =
                getValueSafelyFromPropertyList(properties, PROPERTY_SELF_SIGNED_OR_CLIENT_CERT, Boolean.class, false);
        boolean useClientCert =
                getValueSafelyFromPropertyList(properties, PROPERTY_USE_CLIENT_CERT, Boolean.class, false);
        String authorityOverride =
                getValueSafelyFromPropertyList(properties, PROPERTY_AUTHORITY_OVERRIDE, String.class, null);
        ...
        if (useTLS && (useSelfSigned || useClientCert)) {
            ...
            if (useClientCert) {
                UploadedFile privateKey =
                        getUploadedFileFromPropertyList(properties, "private.key", PROPERTY_CLIENT_PRIVATE_KEY);
                UploadedFile certChain =
                        getUploadedFileFromPropertyList(properties, "client.pem", PROPERTY_CLIENT_CERT_CHAIN);
                if (privateKey != null && certChain != null) {
                    ...
                    tlsBuilder.keyManager(
                            new ByteArrayInputStream(certChain.getDecodedContent()),
                            new ByteArrayInputStream(privateKey.getDecodedContent()));
                }
            }
        ...
        if (useTLS && useClientCert && authorityOverride != null && !authorityOverride.isEmpty()) {
    ...
    StreamObserver<ServerReflectionRequest> buildReflectionStub(DatasourceConfiguration configuration) {
        List<Property> properties = datasourceConfiguration.getProperties();
        int reflectionTimeout = Integer.parseInt(
                getValueSafelyFromPropertyList(properties, PROPERTY_INDEX_REFLECTION_TIMEOUT, String.class, "10"));
    ...
    public Set<String> getServices() {
        ...
        return endpoints.get().stream()
                .map(endpoint -> endpoint.getMethod().getService().getFullName())
    ...
    public Set<String> getEndpoints(String service) {
        return endpoints.get().stream()
                .filter(endpoint ->
                        endpoint.getMethod().getService().getFullName().equals(service))
                .map(endpoints -> endpoints.getMethod().getName())
                .collect(Collectors.toSet());
    }
    ...
    List<GrpcEndpoint> awaitCompletion() {
        ...
        fileDescriptorProtos
                .values()
                .forEach(proto -> makeFileDescriptorPojo(proto.getName(), fileDescriptorProtos, fileDescriptors));
        ...
        for (FileDescriptor fd : fileDescriptors.values())
            for (var svc : fd.getServices()) for (var method : svc.getMethods()) result.add(new GrpcEndpoint(method));
    ...
    public void onNext(ServerReflectionResponse response) {
        ...
        if (response.hasListServicesResponse()) {
            ...
            response.getListServicesResponse().getServiceList().stream()
                    ...
                    .map(name -> ServerReflectionRequest.newBuilder()
                            .setFileContainingSymbol(name)
                            .build())
                    .forEach(this::send);
        } else if (response.hasFileDescriptorResponse()) {
            log.debug("FileDescriptorResponse {}", response.getFileDescriptorResponse());
            // If the GRPC reflection service echos the request in the response, match it up against the requests we
            // sent.  Otherwise
                ...
                if (!outstandingRequests.remove(response.getOriginalRequest())) {
                    error = Status.INTERNAL
                            .withDescription(format(
                                    "Can't find reflection response %s among outstanding requests %s",
                                    response, outstandingRequests))
                            .asRuntimeException();
                }
                ...
                for (ByteString bytes : response.getFileDescriptorResponse().getFileDescriptorProtoList()) {
                    try {
                        ...
                        fd.getDependencyList().stream()
                                .map(dep -> ServerReflectionRequest.newBuilder()
                                        .setFileByFilename(dep)
                                        .build())
                                .forEach(this::send);
            ...
            error = Status.fromCodeValue(err.getErrorCode())
                    .withDescription(err.getErrorMessage())
                    .asRuntimeException();

        } else {
            error = Status.INTERNAL
                    .withDescription(format("Unexpected reflection response: %s", response))
                    .asRuntimeException();
        }

        // If we've received responses to everything we asked for, or if responses surprised us, close the request
        // stream.
     ...
     */
    private static FileDescriptor makeFileDescriptorPojo(
            String name, Map<String, FileDescriptorProto> input, Map<String, FileDescriptor> output) {
        FileDescriptor fd = output.get(name);
        if (fd == null) {
            try {
                FileDescriptorProto proto = input.get(name);
                fd = FileDescriptor.buildFrom(
                        proto,
